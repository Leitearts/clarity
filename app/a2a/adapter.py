...
class A2AAdapter:
    ...
    async def invoke(self, request: A2ARequest, include_full: bool = False) -> A2AResponse:
        logger.info("a2a.invoke trace_id=%s caller=%s capability=%s",
                    request.trace_id, request.caller_id, request.capability)
        handler = self._capability_router(request.capability)
        if handler is None:
            return A2AResponse(trace_id=request.trace_id, status=A2AStatus.REJECTED,
                               analysis_id="", case_id="",
    ...
    def _capability_router(self, capability: str):
        return {"clinical_risk_analysis": self._handle_full_analysis,
                "drug_interaction_check": self._handle_drug_check,
                "lab_interpretation":     self._handle_lab_check}.get(capability)

    async def _handle_full_analysis(self, request: A2ARequest, include_full: bool) -> A2AResponse:
        try:
            case = PatientCase(**request.payload)
        except (ValidationError, TypeError) as exc:
            return A2AResponse(trace_id=request.trace_id, status=A2AStatus.REJECTED,
                               analysis_id="", case_id=request.payload.get("case_id", ""),
        ...
        try:
            ...
            return A2AResponse(trace_id=request.trace_id, status=A2AStatus.ERROR,
                               analysis_id="", case_id=case.case_id,
        ...
        agent_contributions = [
            AgentContributionSummary(
                ...
                contribution_pct=round((weight_map.get(r.agent_type.value, 0.0) * r.risk_score / ws) * 100, 1),
        ...
        return A2AResponse(
            trace_id=request.trace_id, status=status,
            analysis_id=analysis.analysis_id, case_id=analysis.case_id,
    ...
    async def _handle_drug_check(self, request: A2ARequest, include_full: bool) -> A2AResponse:
        ...
        agent_request = AgentRequest(agent_type=AgentType.DRUG_INTERACTION,
        ...
        try:
            ...
            return A2AResponse(trace_id=request.trace_id, status=A2AStatus.ERROR,
                               ...
                               error_code="AGENT_FAILED", error_message=str(exc))
        return A2AResponse(
            trace_id=request.trace_id, status=A2AStatus.SUCCESS,
            analysis_id=f"drug-{request.trace_id}", case_id=case_id,
            risk_score=result.risk_score, risk_level=_score_to_level(result.risk_score),
            narrative=result.reasoning,
            critical_findings=[f.description for f in result.findings if f.severity.value in ("critical", "high")],
            full_result=result.model_dump() if include_full else None,
        )

    async def _handle_lab_check(self, request: A2ARequest, include_full: bool) -> A2AResponse:
        ...
        agent_request = AgentRequest(agent_type=AgentType.LAB_ANALYSIS,
                                     case_id=case_id, patient_id=patient_id,
        ...
        return A2AResponse(
            trace_id=request.trace_id, status=A2AStatus.SUCCESS,
            analysis_id=f"lab-{request.trace_id}", case_id=case_id,
            risk_score=result.risk_score, risk_level=_score_to_level(result.risk_score),
            narrative=result.reasoning,
            critical_findings=[f.description for f in result.findings if f.severity.value in ("critical", "high")],
            full_result=result.model_dump() if include_full else None,

...
def validate_input(case: PatientCase) -> None:
    ...
    if len(case.medications) > MAX_MEDICATIONS:
        raise SafetyError(
            f"Medication count {len(case.medications)} exceeds maximum {MAX_MEDICATIONS}."
        )
    if len(case.diagnoses) > MAX_DIAGNOSES:
        raise SafetyError(
            f"Diagnosis count {len(case.diagnoses)} exceeds maximum {MAX_DIAGNOSES}."
        )
    if len(case.lab_results) > MAX_LAB_RESULTS:
        raise SafetyError(
            f"Lab count {len(case.lab_results)} exceeds maximum {MAX_LAB_RESULTS}."
        )
...
def sanitize_output(response: AnalysisResponse) -> AnalysisResponse:
    ...
    if flagged:
        logger.warning(
            "safety.output_flagged analysis_id=%s flags=%s",
...
def append_disclaimer(response: AnalysisResponse) -> AnalysisResponse:
    return response.model_copy(
        update={"explanation": response.explanation + DISCLAIMER}
    )

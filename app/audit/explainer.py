...
class ExplanationPayload:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "narrative": self.narrative, "risk_level": self.risk_level,
            "risk_score": self.risk_score, "dominant_factor": self.dominant_factor,
            "formula_trace": self.formula_trace,
            "all_critical_findings": self.all_critical_findings,
            "recommended_actions": self.recommended_actions,
            "agent_breakdown": [
                {"agent": ae.agent_name, "risk_score": ae.risk_score,
                 "weight": ae.weight, "weighted_contribution": ae.weighted_contribution,
                 "contribution_pct": ae.contribution_pct,
                 "findings_summary": ae.findings_summary, "reasoning": ae.reasoning,
                 "had_error": ae.had_error}
                for ae in self.agent_explanations
            ],
            "model_version": self.model_version, "analysis_id": self.analysis_id,
        }


class ExplanationBuilder:
    def build(self, analysis_id: str, risk: RiskScore,
              agent_responses: list[AgentResponse],
              recommended_actions: list[str],
              model_version: str = "clarity-v1.0") -> ExplanationPayload:
        weight_map = {
        ...
        for resp in agent_responses:
            ...
            agent_explanations.append(AgentExplanation(
                ...
                risk_score=resp.risk_score, weight=weight,
                weighted_contribution=wc, contribution_pct=pct,
                findings_summary=[f.description for f in resp.findings],
                critical_findings=[f.description for f in resp.findings
                                    if f.severity in (FindingSeverity.CRITICAL, FindingSeverity.HIGH)],
                reasoning=resp.reasoning, had_error=resp.error is not None,
            ))
        ...
        all_critical = [f"[{ae.agent_name.upper()}] {f}"
                        for ae in agent_explanations for f in ae.critical_findings]
        formula_trace = (
        ...
        narrative = _build_narrative(risk, agent_explanations, dominant_name,
                                      all_critical, recommended_actions)
        return ExplanationPayload(
            narrative=narrative, risk_level=risk.level.value,
            risk_score=risk.total_score, dominant_factor=dominant_name,
            agent_explanations=agent_explanations, formula_trace=formula_trace,
            all_critical_findings=all_critical, recommended_actions=recommended_actions,
...
def _build_narrative(risk: RiskScore, agent_explanations: list[AgentExplanation],
                     dominant_name: str, all_critical: list[str],
    ...
    for ae in agent_explanations:
        if ae.weight == 0:
            continue
        if ae.had_error:
            parts.append(f"The {ae.agent_name} agent was unavailable (fallback score: {ae.risk_score:.2f}).")
        elif ae.risk_score >= 0.65:
            parts.append(f"The {ae.agent_name} agent returned a high score of {ae.risk_score:.2f} ({ae.contribution_pct:.0f}% of total): {ae.reasoning}")
        elif ae.risk_score >= 0.35:
            parts.append(f"The {ae.agent_name} agent flagged moderate risk ({ae.risk_score:.2f}): {ae.reasoning}")
        else:
            parts.append(f"The {ae.agent_name} agent found minimal concerns (score {ae.risk_score:.2f}).")
    if risk.context_multiplier != 1.0:
        direction = "increased" if risk.context_multiplier > 1.0 else "reduced"
        ctx = next((ae for ae in agent_explanations if ae.agent_name == "patient context"), None)
        ctx_reasoning = ctx.reasoning if ctx else ""
        parts.extend(["", f"Patient context {direction} the score by ×{risk.context_multiplier:.2f}. {ctx_reasoning}"])
    if all_critical:
        count = len(all_critical)
        parts.extend(["", f"{count} critical/high finding{'s' if count > 1 else ''}: "
                      + "; ".join(all_critical[:3])

...
@router.get(
    "/impact",
    summary="Real-time CLARITY usage and impact statistics",
...
async def get_impact(request: Request):
    ...
    avg_ms = sum(r.processing_time_ms for r in records if r.processing_time_ms) / max(
        sum(1 for r in records if r.processing_time_ms), 1
    )

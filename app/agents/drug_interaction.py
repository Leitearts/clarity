            ...
            try:
                ...
                for interaction in result.get("interactions", []):
                    ...
                    severity = _SEVERITY_MAP.get(
                        interaction.get("severity", "medium").lower(), FindingSeverity.MEDIUM
                    )
                    findings.append(Finding(
                        code="DDI_" + "_".join(d.upper() for d in drugs[:2]),
                        description=interaction.get("description", f"Interaction: {', '.join(drugs)}"),
                        severity=severity,
                ...
                return self._build_response(
                    ...
                    findings=findings,
                    reasoning=result.get("reasoning", "LLM drug safety assessment completed."),

...
def vitals_rules(request: AgentRequest, vitals: list[VitalSign], agent) -> AgentResponse:
    ...
    for vital in vitals:
        ...
        if vital.is_abnormal:
            abnormal_count += 1
            direction = "low" if (vital.reference_low and vital.value < vital.reference_low) else "high"

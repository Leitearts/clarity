        ...
        async def side_effect(system_prompt, user_prompt, required_keys, **kwargs):
            ...
            if is_diagnosis:
                if "I21" in prompt_text or "I21.9" in prompt_text:  # Acute MI
                    ...
                    "J18.9" in prompt_text
                ):  # Pneumonia - serious, especially with allergy conflict
                    data["risk_score"] = 0.80

            # DRUG INTERACTION AGENT
            elif is_drug:
                if (
                    "warfarin" in prompt_text.lower()
                    and "amiodarone" in prompt_text.lower()
                ):
                    ...
                    "amoxicillin" in prompt_text.lower()
                    and "penicillin" in prompt_text.lower()
                ):
                    # Allergy conflict: amoxicillin is a penicillin-based antibiotic
                    data["risk_score"] = 0.95
                    data["allergy_conflicts"] = ["amoxicillin"]
                elif (
                    "fluoxetine" in prompt_text.lower()
                    and "tramadol" in prompt_text.lower()
                ):
                    ...
                    prompt_text.count("drug_") >= 5 or prompt_text.count('"name"') >= 5
                ):  # Polypharmacy
            ...
            # LAB ANALYSIS AGENT
                ...
                if (
                    "hemoglobin" in prompt_text.lower()
                    and "14.5" in prompt_text
                    and "potassium" in prompt_text.lower()
                    and "4.1" in prompt_text
                ):
                ...
                # Detect specific abnormal values
                ...
                elif "troponin" in prompt_text.lower() and (
                    "2.1" in prompt_text or "high" in prompt_text.lower()
                ):
                    # Critical troponin
                    data["risk_score"] = 0.90
                    data["critical_values"] = [
                        {"test": "troponin", "value": 2.1, "unit": "ng/mL"}
                    ]
                elif "potassium" in prompt_text.lower() and (
                    "6.2" in prompt_text
                    or "2.3" in prompt_text
                    or "2.5" in prompt_text
                    or "6.8" in prompt_text
                ):
                    # Critical potassium
                    data["risk_score"] = 0.85
                    critical_val = (
                        2.3
                        if "2.3" in prompt_text
                        else 2.5
                        if "2.5" in prompt_text
                        else 6.2
                        if "6.2" in prompt_text
                        else 6.8
                    )
                    data["critical_values"] = [
                        {"test": "potassium", "value": critical_val, "unit": "mEq/L"}
                    ]
                    ...
                    data["abnormal_values"] = (
                        ["WBC"] if "wbc" in prompt_text.lower() else ["hemoglobin"]
                    )
            ...
            # PATIENT CONTEXT AGENT
                ...
                elif "age" in prompt_text.lower() and (
                    "icu" in prompt_text.lower() or "emergency" in prompt_text.lower()
                ):
                    # Handle other elderly/high-acuity settings
                    if any(
                        age_str in prompt_text for age_str in ["71", "64", "65", "78"]
                    ):

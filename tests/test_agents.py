...
class TestDrugInteractionAgent:
    ...
    async def test_warfarin_amiodarone_is_critical(self, mock_llm):
        ...
        response = await agent.run(request)
        critical = [
            f for f in response.findings if f.severity == FindingSeverity.CRITICAL
        ]
    ...
    async def test_ssri_tramadol_serotonin_risk(self, mock_llm):
        ...
        serotonin = [
            f
            for f in response.findings
            if "serotonin" in f.description.lower()
            or f.severity == FindingSeverity.CRITICAL
        ]
